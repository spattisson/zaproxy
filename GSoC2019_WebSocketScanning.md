...
1.  [WebSocket Scanning (Active - Passive)](#org11bf78d)
    1.  [Project](#org8f4100e)
    2.  [WebSocket Passive Scanning [iss#5346]](#org5a9c4ae)
        1.  [Completed Work](#org0d877b2)
        2.  [Enhancements / To be done / Optional](#org186871a)
    3.  [WebSocket Tree Map [iss#5539]](#org32185b2)
        1.  [Pending Work](#org888a9e5)
    4.  [WebSocket Active Scanning [iss#4276]](#org3394470)
2.  [Weekly Update](#orgf92f973)
...
Mentors: @psiinon, @kingthorin<sub>rm</sub>, @thc202
...
we want create those tools which are going to give ZAP the ability to perform
...
our testing parameters. That's complicates the procedure is that the
...
addition, some of the payloads might not even follows a formal format (e.g.
...
procedure to use and write your own passive scan script rules in ZAP.
...
-   [WS-Passive-Scan] Add WebSocket passive scripts by default ([PR#2089](https://github.com/zaproxy/zap-extensions/pull/2089))
    -   Loads the default script (following ones) at ZAP's start up.
...
-   [WS-Passive-Scan] Override getName & getId ([PR#2104](https://github.com/zaproxy/zap-extensions/pull/2104))
    -   Provide to scripts the ability to used different names and IDs.
...
-   Minors:
    ...
    -   [WS-Passive-Scan] Add websocketpassive script type in build.gradle.kts ([[%5BWS-Passive-Scan%5D Add websocketpassive script type in

build.gradle.kts][PR#162]])
...
-   Implement API
-   Even More are in the Tracking Issue.
...
-   Infrastructure for WebSocket Tree Map ([PR#2135](https://github.com/zaproxy/zap-extensions/pull/2135)) - Reviewing
    -   This PR is a refactored one based on [PR#1742](https://github.com/zaproxy/zap-extensions/pull/1742) which was created last year. In
        ...
        -   Every time a new Message has been transmitted oven an active channel the
...
-   Tech Specs for WebSocket TreeMap and Payload Analyzers ([TreeMap Tech Specs](https://docs.google.com/document/d/1TllW7AkTUZHc5Au7fQDYQLMTfOiYwm-luhJNiSn1xvo/edit))
...
could not make it to adapt the existing Active Scanner infrastructure
